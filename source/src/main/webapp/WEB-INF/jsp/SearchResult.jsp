	...
	<div class="wrapper">
		...
		<form method="get" action="SearchResultServlet"
			...
			<input type="hidden" name="name" value="${name}" /> <input
				type="hidden" name="furigana" value="${furigana}" /> <input
				type="hidden" name="schoolName" value="${schoolName}" /> <input
					...
					<c:forEach var="s" items="${studentList}">
						<div class="student-card-wrapper" style="position: relative;">
							...
							<form action="SearchResultServlet" method="post"
								...
								<input type="hidden" name="deleteId" value="${s.id}" /> <input
						...
						<c:if test="${currentPage > 1}">
							<form method="get" action="SearchResultServlet"
								...
								<input type="hidden" name="name" value="${name}" /> <input
									...
									type="hidden" name="page" value="${currentPage - 1}" /> <input
									type="submit" value="前へ" />
							</form>
						</c:if>

						<!-- 現在のページ番号表示 -->
						<span style="margin: 0 15px; font-weight: bold;">${currentPage}
							/ ${totalPages}</span>
